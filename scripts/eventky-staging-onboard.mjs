...
const context = await chromium.launchPersistentContext(profile, {headless:true,viewport:{width:1440,height:1000},timezoneId:'Europe/Zurich'});
...
try {
 ...
 if(config.homeserver!=='ufibwbmed6jeq9k4p583go95wofakh9fwpp4k734trq79pd9u1uy'||config.deployEnv!=='staging') throw new Error('Refuse non-staging app');
 ...
 await fs.writeFile(`/tmp/eventky-staging-${role}-public.json`,JSON.stringify({pubky,origin,homeserver:config.homeserver}),{mode:0o600});
 await page.screenshot({path:`/tmp/eventky-staging-${role}-home.png`,fullPage:true});
 console.log('Staging onboarding passed',pubky);
} catch(error){console.error(String(error)); console.error('URL',page.url()); console.error((await page.locator('body').innerText({timeout:3000}).catch(()=>'' )).slice(-3000)); process.exitCode=1;}
