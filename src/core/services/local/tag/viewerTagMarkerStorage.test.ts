...
describe('ViewerTagMarkerStorage', () => {
  ...
  it('cleans expired markers on subsequent writes without relying on tag-list reconciliation', () => {
    vi.useFakeTimers();
    vi.setSystemTime(1_000_000);
    ViewerTagMarkerStorage.set({ pubky: VIEWER_A, taggedId: POST_ID, label: 'old', op: HttpMethod.PUT });
    vi.advanceTimersByTime(MARKER_TTL_MS + 1);
    ViewerTagMarkerStorage.set({ pubky: VIEWER_A, taggedId: POST_ID, label: 'new', op: HttpMethod.PUT });

    expect(window.sessionStorage.getItem(buildKey(VIEWER_A, POST_ID, 'old'))).toBeNull();
    expect(ViewerTagMarkerStorage.get({ pubky: VIEWER_A, taggedId: POST_ID, label: 'new' })?.op).toBe(HttpMethod.PUT);
  });
