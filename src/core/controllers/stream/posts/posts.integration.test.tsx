...
describe('StreamPostsController - Integration', () => {
  ...
  it('publishes cold reply IDs after grouped hydration, so mounting replies needs no individual requests', async () => {
    ...
    const postFetch = vi.spyOn(NexusPostService, 'getPost').mockResolvedValue(reply);
    ...
    try {
      ...
      const published = await StreamPostsController.getLocalStream({ streamId });
      expect(published?.stream).toEqual(['author:reply']);
      await vi.waitFor(() => expect(observed).toContain('author:reply'));
      expect(incomplete).toEqual([]);
      const [replyId] = published?.stream ?? [];
      const hooks = renderHook(() => ({
        details: usePostDetails(replyId),
        counts: usePostCounts(replyId),
        tags: usePostTags(replyId),
      }));
